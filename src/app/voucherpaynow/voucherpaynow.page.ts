...
@Component({
  selector: 'app-voucherpaynow',
  templateUrl: './voucherpaynow.page.html',
  styleUrls: ['./voucherpaynow.page.scss'],
  
})
export class VoucherpaynowPage implements OnInit
{
  ...
  async Process()
  {
    ...
    if ( this.auth.DealResult != undefined && this.auth.DealResult.Code == 1 )
    {
      ...
      var body = `TradingPartnerId=${this.auth.VoucherDeal.TradingPartnerId}&CampaignPurchaseId=${this.auth.DealResult.CampaignPurchaseId}&MemberId=${this.auth.CurrentUser.Member.Id}&CampaignId=${this.auth.VoucherDeal.Id}&Quantity=1&Amount=${this.auth.DealAmount}&Discount=${this.auth.VoucherDeal.CampaignTier.DiscountPercentage || 0}&PaymentMethod=${this.auth.DealPaymentMethod}&AccessToken=${this.auth.APIAuthentication.AccessToken}`;
  ...
  async CheckPaymentStatus()
  {
    ...
    if ( this.auth.PaymentResult != undefined && this.auth.PaymentResult.Code == 1 )
    {
      ...
      this.navCtrl.navigateForward( "voucherpaymentsuccess" );
    }
    else if ( this.auth.PaymentResult != undefined && this.auth.PaymentResult.Code == 0 )
    {
      this.navCtrl.navigateForward( "voucherpaymentfailed" );
    }
  }
  ...
  async Confirm()
  {
    ...
    ({
      ...
      component: VoucherpayconfirmComponent
    });
