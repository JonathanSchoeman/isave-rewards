...
export class ChatService
{
  ...
  async GetDepartments()
  {
    try
    {
      ...
      var url = `${this.auth.APIUrl}/api/Chat/GetDepartments?apikey=${this.auth.APIKey}&ClientId=${this.auth.ClientId}`;
  ...
  async List( query:string = "" )
  {
    ...
    try
    {
      var url = `${this.auth.APIUrl}/api/Chat/List?UserId=${this.auth.CurrentUser.Id}&ClientId=${this.auth.ClientId}&apikey=${this.auth.APIKey}`;
  ...
  async Send( chat: any, message: string = "" )
  {
    ...
    try
    {
      chat = `${chat}&UserId=${this.auth.CurrentUser.Id}&ClientId=${this.auth.ClientId}&SenderUserId=${this.auth.CurrentUser.Id}&apikey=${this.auth.APIKey}`;
  ...
  async RefreshChats()
  {
    ...
    {
      var url = `${me.auth.APIUrl}/api/Chat/List?UserId=${me.auth.CurrentUser.Id}&ClientId=${me.auth.ClientId}&apikey=${me.auth.APIKey}`;
  ...
  async RefreshTicket( ticketId: number, content: any )
  {
    ...
    var x = me.Chats.findIndex( (n: { Id: any; }) => n.Id == me.Chat.Id );

    me.Timeout = setInterval( async function ()
    {
      var url = `${me.auth.APIUrl}/api/Chat/Receive?UserId=${me.auth.CurrentUser.Id}&ClientId=${me.auth.ClientId}&apikey=${me.auth.APIKey}&ticketId=${ticketId}`;

      var ticket = await me.auth.http.get<iCommonModel>( url ).toPromise();

      if ( ticket && me.Chats[ x ].Messages.length < ticket.Messages.length )
      {
  ...
  async MarkAsRead( ticketId: number )
  {
    ...
    try
    {
      var p = `UserId=${this.auth.CurrentUser.Id}&ClientId=${this.auth.ClientId}&apikey=${this.auth.APIKey}&ticketId=${ticketId}&receiverUserId=${this.auth.CurrentUser.Id}`;
